...
namespace dxvk {
  ...
  DxvkInstance::DxvkInstance() {
    ...
    // HUD settings
    if (!re4t::dxvk::cfg->DXVK_HUD.empty())
    {
        m_config.setOption("dxvk.hud", re4t::dxvk::cfg->DXVK_HUD);
    }
    else
    {
        std::string DXVK_HUD = "";

        if (re4t::dxvk::cfg->bShowFPS)
            DXVK_HUD += "fps,";

        if (re4t::dxvk::cfg->bShowGPULoad)
            DXVK_HUD += "gpuload,";
        
        if (re4t::dxvk::cfg->bShowDeviceInfo)
            DXVK_HUD += "devinfo,";

        DXVK_HUD = DXVK_HUD.substr(0, DXVK_HUD.size() - 1);

        m_config.setOption("dxvk.hud", DXVK_HUD);
    }
