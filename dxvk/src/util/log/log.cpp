...
namespace dxvk {
  ...
  LogLevel Logger::getMinLogLevel() {
    ...
    iniReader ini;
    const std::string logLevelStr = ini.getString("RENDERER", "DXVK_LOG_LEVEL", "info");
  ...
  std::string Logger::getFileName(const std::string& base) {
    std::string path = env::getExePath();
    path = path.substr(0, path.find_last_of("/\\")) + "\\re4_tweaks\\dxvk.log";
